...
class UserConsumer(AsyncWebsocketConsumer):
    ...
    def check_user(self) -> Optional[Literal[True]]:
        return UserModel.get_all().filter(email=self.scope["user"]).exists()

    @database_sync_to_async
    def check_user_group_name(self) -> Optional[Literal[True]]:
        user: UserModel = UserModel.get_all().filter(email=self.scope["user"])
        return user[0].group_name == self.room_group_name

    @database_sync_to_async
    def room_groop_name(self) -> UserModel:
        return UserModel.objects.get(email=self.scope["user"]).group_name

    @database_sync_to_async
    def add_user_to_active(self) -> None:
        self.disconnect(200)
        user: UserModel = UserModel.objects.get(email=self.scope["user"])
        user.is_online = True
        user.save()
