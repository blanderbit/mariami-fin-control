...
class VerifyCodeValidator:
    ...
    def __call__(self, attrs: OrderedDict) -> OrderedDict:
        ...
        self.code = VerifyCodeModel.objects.filter(verify_code=self.verify_code)
        if not self.code:
            raise ValidationError(BAD_CODE_ERROR, HTTP_400_BAD_REQUEST)
        elif VerifyCodeModel.objects.get(verify_code=self.verify_code).type not in self.token_type:
            raise ValidationError(BAD_CODE_ERROR, HTTP_400_BAD_REQUEST)
        elif VerifyCodeModel.objects.get(verify_code=self.verify_code).life_time < timezone.now():
